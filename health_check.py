...
def _memory_from_sysconf() -> Optional[Tuple[int, int]]:
    ...
    return int(total_pages * page_size), int(available_pages * page_size)


def _memory_from_windows() -> Optional[Tuple[int, int]]:
    if os.name != "nt":
        return None
...
def _fallback_memory_usage() -> Tuple[str, str, float]:
    memory = _memory_from_sysconf() or _memory_from_windows()
    if memory is None:
        return "WARNING", "Cannot check: memory information unavailable on this platform", 0
...
def check_memory_usage() -> Tuple[str, str, float]:
    try:
        ...
        status, detail, pct = _fallback_memory_usage()
        if status != "WARNING" or pct > 0:
            return status, detail, pct
        return "WARNING", f"Cannot check: {e}", 0


def _load_status(load: float, cpu_count: int) -> Tuple[str, str, float]:
...
def _fallback_load_average() -> Tuple[str, str, float]:
    try:
        load = os.getloadavg()[0]
    except (AttributeError, OSError):
        return "WARNING", "Cannot check: load average unavailable on this platform", 0

    cpu_count = os.cpu_count() or 1
    return _load_status(load, cpu_count)


def check_load_average() -> Tuple[str, str, float]:
    try:
        with builtins.open("/proc/loadavg") as f:
            parts = f.read().strip().split()
            load = float(parts[0])
            cpu_count = os.cpu_count() or 1
            return _load_status(load, cpu_count)
    except Exception as e:
        status, detail, load = _fallback_load_average()
        if status != "WARNING" or load > 0:
