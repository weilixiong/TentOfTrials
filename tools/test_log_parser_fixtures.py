...
def main() -> int:
    ...
    # Empty / whitespace-only lines must not crash (text parser returns None)
    for blank in ("", "   ", "\t"):
        try:
            if jp.parse(blank) not in (None,):
                # JSON empty is None; OK if None
                pass
            tp.parse(blank)
            np.parse(blank)
        except Exception as e:
            failures.append(f"blank line crashed parsers: {e!r}")

    # Fixture files must be hand-authored: no auto-generated marker
    for name in (
        "json_lines.log",
        "text_lines.log",
        "nginx_lines.log",
        ...
        if "GENERATED BY" in raw.upper() or "auto-generated" in raw.lower():
            failures.append(f"fixture looks generated: {name}")

    # Service/format keys present on good nginx + json samples
    sample_json = jp.parse(load_lines("json_lines.log")[0])
    if not sample_json or sample_json.get("service") != "api":
        failures.append(f"json service expected api, got {sample_json}")
    sample_ngx = np.parse(load_lines("nginx_lines.log")[0])
    if not sample_ngx or sample_ngx.get("fields", {}).get("status") != 200:
        failures.append(f"nginx status expected 200, got {sample_ngx}")
