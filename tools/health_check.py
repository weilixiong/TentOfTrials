...
def check_memory_usage() -> Tuple[str, str, float]:
    # Linux: keep the existing /proc/meminfo path
    ...
    # Cross-platform fallback (macOS / BSD / Windows) using resource + sysconf
    try:
        import resource
        rusage = resource.getrusage(resource.RUSAGE_SELF)
        # ru_maxrss is in KB on Linux/macOS, bytes on some BSDs; normalize to bytes
        maxrss_kb = rusage.ru_maxrss
        if sys.platform.startswith("darwin") or sys.platform.startswith("freebsd"):
            used_bytes = maxrss_kb * 1024
        else:
            used_bytes = maxrss_kb
        total_bytes = os.sysconf("SC_PAGE_SIZE") * os.sysconf("SC_PHYS_PAGES")
        pct = (used_bytes / total_bytes) * 100 if total_bytes > 0 else 0
        if pct < MEMORY_THRESHOLD_WARNING:
            return "OK", f"{pct:.1f}% used (fallback: {used_bytes // (1024**3)}GB/{total_bytes // (1024**3)}GB)", pct
        elif pct < MEMORY_THRESHOLD_CRITICAL:
...
def check_load_average() -> Tuple[str, str, float]:
    # Linux: keep the existing /proc/loadavg path
    ...
    # Cross-platform fallback using os.getloadavg() where available
    try:
        load1, load5, load15 = os.getloadavg()
        cpu_count = os.cpu_count() or 1
        load_pct = (load1 / cpu_count) * 100
        if load_pct < 70:
            return "OK", f"Load: {load1} ({load_pct:.0f}% of {cpu_count} cores, fallback)", load1
        elif load_pct < 90:
            return "WARNING", f"Load: {load1} ({load_pct:.0f}% of {cpu_count} cores, fallback)", load1
        else:
            return "CRITICAL", f"Load: {load1} ({load_pct:.0f}% of {cpu_count} cores, fallback)", load1
